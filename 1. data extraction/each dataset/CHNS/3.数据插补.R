...
rm(list=ls())
library(openxlsx)
varechem <- read.xlsx("original data/matched_data.xlsx")###记得删掉psm的三个参数
varechem$Status <- as.factor(varechem$Status)
summary(varechem$Status)
#varechem <- varechem %>% filter(!is.na(varechem$`Triglycerides.(mg_dL)`))
# 数据检视
varechem %>% glimpse()
# 自定义缺失率计算函数
...
new_colnames <- paste0("index_", 1:29)
