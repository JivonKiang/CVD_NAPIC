...
#载入数据
# 读取数据
data1 <- readRDS("result/completedData.RDS")
...
index <- index[,-(1:12)]###去掉index中前面的用于计算新指标的生理生化指标
data <- cbind(data1,index)
data <- data[,-(4:20)]###去掉分类变量,性别是需要被纳入降维的
data_colname <- as.data.frame(colnames(data))
colnames(data)[1:41486] <- paste0("index_", 1:41486)###改一下起始点
